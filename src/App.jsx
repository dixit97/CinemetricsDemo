...
// --- Custom Aperture Logo Component (Matches your upload) ---
...
export default function App() {
  ...
  const [view, setView] = useState('home'); // 'home', 'dashboard', 'guest'
  const [activeTab, setActiveTab] = useState('dashboard'); // 'dashboard', 'leads', 'automations'
      ...
      <div className="min-h-screen bg-[#FAF9F6] text-[#1A1A1A] font-sans selection:bg-[#D4AF37] selection:text-white">
        <nav className="fixed top-0 w-full z-50 px-6 md:px-12 py-6 flex justify-between items-center bg-white/80 backdrop-blur-xl border-b border-stone-100">
          ...
          <div className="flex items-center gap-6">
            <button onClick={() => setView('dashboard')} className="text-xs font-bold uppercase tracking-widest text-stone-500 hover:text-black transition-all hidden md:block">Sign In</button>
            <button onClick={() => setView('dashboard')} className="px-6 py-3 bg-[#1A1A1A] text-white rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-black shadow-xl hover:shadow-2xl transition-all">Launch Engine</button>
          </div>
        </nav>
        ...
        <section className="relative pt-48 pb-32 px-8 flex flex-col items-center text-center">
          ...
          <div className="max-w-4xl relative z-10 space-y-10 animate-in fade-in slide-in-from-bottom-8 duration-1000">
            ...
            <div className="flex justify-center gap-6 pt-4">
              <button onClick={() => setView('dashboard')} className="px-10 py-5 bg-[#D4AF37] text-black rounded-2xl text-sm font-bold uppercase tracking-widest hover:bg-[#C4A030] shadow-[0_0_40px_rgba(212,175,55,0.3)] transition-all">
                Access Platform
              </button>
            </div>
          </div>
  ...
  if (view === 'guest' && galleryContext) {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 transition-colors duration-1000" style={{ backgroundColor: theme.bg, color: theme.text }}>
        <button onClick={() => setView('dashboard')} className="fixed top-8 left-8 flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest opacity-40 hover:opacity-100 transition-opacity">
          <ChevronLeft size={14} /> Photographer Hub
        </button>
          ...
          {gateStep === 2 && (
            <div className="space-y-8 animate-in slide-in-from-right-4 duration-500 text-left">
              ...
              <div className="space-y-5">
                <input required type="text" placeholder="Full Name" className="w-full p-5 bg-stone-50 border border-stone-100 rounded-2xl text-sm outline-none text-black transition-all focus:border-[#D4AF37]" value={guestData.name} onChange={e => setGuestData({...guestData, name: e.target.value})} />
                <input required type="email" placeholder="Email Address" className="w-full p-5 bg-stone-50 border border-stone-100 rounded-2xl text-sm outline-none text-black transition-all focus:border-[#D4AF37]" value={guestData.email} onChange={e => setGuestData({...guestData, email: e.target.value})} />
                <button onClick={() => setGateStep(3)} className="w-full py-5 rounded-2xl font-bold uppercase tracking-widest text-xs text-white bg-black hover:bg-stone-900 transition-all flex justify-center items-center">
          ...
          {gateStep === 3 && (
            <div className="space-y-8 animate-in slide-in-from-right-4 duration-500">
              ...
              <div className="space-y-4">
                <button onClick={() => finalizeLead('Wedding')} className="w-full p-5 bg-black text-white rounded-3xl flex items-center justify-between group hover:shadow-2xl transition-all border border-black">
                ...
                <button onClick={() => finalizeLead('Future')} className="w-full p-5 bg-white border border-stone-200 rounded-3xl flex items-center justify-between group hover:border-[#D4AF37] transition-all" style={{ backgroundColor: isDark ? '#222' : '#FFF', borderColor: isDark ? '#333' : '#E5E5E5' }}>
          ...
          {gateStep === 4 && (
            <div className="space-y-10 animate-in zoom-in-95 duration-700 text-center py-6">
              ...
              <div className="space-y-4">
                ...
                <p className="text-sm opacity-60 leading-relaxed px-2 font-light">Your $200 credit is banked. We have securely emailed your private gallery link to <span className="font-bold underline" style={{ color: theme.text }}>{guestData.email}</span>.</p>
              </div>
